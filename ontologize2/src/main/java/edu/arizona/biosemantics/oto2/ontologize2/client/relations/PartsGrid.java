...
public class PartsGrid extends MenuTermsGrid {
	...
	public PartsGrid(EventBus eventBus) {
		super("part", "parent", "part", new ValueProvider<Term, String>() {
			@Override 
	...
	protected void addAttachedTermsToRow(Row row, List<Term> add) throws Exception {
		...
		if(valid) {
			for(Term term : add) {
				List<Row> rows = this.getAttachedTermsRows(term);
				for(Row attachedRow : rows) {
					Term parent = attachedRow.getLeadTerm();
					Term disambiguatedTerm = term.clone(false);
					disambiguatedTerm.addDisambiguator(parent.getDisambiguatedValue());
					attachedRow.replaceAttachedTerm(term, disambiguatedTerm);
					eventBus.fireEvent(new DisambiguateTermEvent(term, disambiguatedTerm));
					this.updateRow(attachedRow);
					
				}
				if(!rows.isEmpty())
					term.addDisambiguator(row.getLeadTerm().getDisambiguatedValue());
			}
			super.addAttachedTermsToRow(row, add);
			eventBus.fireEvent(new AddPartEvent(row.getLeadTerm(), add));
		}
	}
